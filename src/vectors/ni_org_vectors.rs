use crate::import::rec_structs::NIOrgRec;
use crate::AppError;
use sqlx::{postgres::PgQueryResult, Pool, Postgres};
use chrono::NaiveDate;

pub struct NIOrgVecs {
...
impl NIOrgVecs{
    ...
    pub fn add_data(&mut self, r: &NIOrgRec) 
    {
    ...
    pub async fn store_data(&self, pool : &Pool<Postgres>) -> Result<PgQueryResult, AppError> {
        ...
        let sql = r#"INSERT INTO ods.ni_orgs (ods_code, ods_name, health_geog, 
                      city, postcode, postal_add, open_date, close_date, subtype_code, status, parent_org) 
            SELECT * FROM UNNEST($1::text[], $2::text[], $3::text[], $4::text[], $5::text[], 
                $6::text[], $7::date[], $8::date[], $9::text[], $10::text[], $11::text[]);"#;

        sqlx::query(&sql)
        .bind(&self.codes).bind(&self.names).bind(&self.health_geogs)
        .bind(&self.cities).bind(&self.postcodes).bind(&self.postal_adds)
        .bind(&self.open_dates).bind(&self.close_dates).bind(&self.subtype_codes).bind(&self.statuses).bind(&self.parent_orgs)
        .execute(pool).await
