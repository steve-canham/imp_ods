...
impl IOMVecs{
    ...
    pub fn add_data(&mut self, r: &IoMRec) 
    {
    ...
    pub async fn store_data(&self, pool : &Pool<Postgres>) -> Result<PgQueryResult, AppError> {
        ...
        let sql = r#"INSERT INTO ods.iom_orgs (ods_code, ods_name, 
...
pub async fn import_data(data_folder: &PathBuf, source_file_name: &str, pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let mut dv: IOMVecs = IOMVecs::new(vector_size);
            
    for result in csv_rdr.deserialize() {
    
        let source: IoMLine = result?;
        ...
        dv.add_data(&iom_rec);   // transfer data to vectors
        i+=1;    
    }
            
    dv.store_data(&pool).await?;
    info!("{} records processed from {} to ods.iom_orgs", i, source_file_name);
