...
impl PHPVecs{
    ...
    pub fn add_data(&mut self, r: &PHPRec) 
    {
    ...
    pub async fn store_data(&self, pool : &Pool<Postgres>) -> Result<PgQueryResult, AppError> {
        ...
        let sql = r#"INSERT INTO ods.php_providers (ods_code, ods_name, grouping, health_geog, 
...
pub async fn import_data(data_folder: &PathBuf, source_file_name: &str, pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let mut dv: PHPVecs = PHPVecs::new(vector_size);
            
    for result in csv_rdr.deserialize() {
    
        let source: PHPLine = result?;
        ...
        dv.add_data(&php_rec);   // transfer data to vectors
        i+=1;    
