...
impl NonNHSVecs{
    ...
    pub fn add_data(&mut self, r: &NonNHSRec) 
    {
    ...
    pub async fn store_data(&self, pool : &Pool<Postgres>) -> Result<PgQueryResult, AppError> {
        ...
        let sql = r#"INSERT INTO ods.non_nhs (ods_code, ods_name, grouping, health_geog, 
...
pub async fn import_data(data_folder: &PathBuf, source_file_name: &str, pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let mut dv: NonNHSVecs = NonNHSVecs::new(vector_size);
            
    for result in csv_rdr.deserialize() {
    
        let source: NonNHSLine = result?;
    ...
    info!("{} records processed from {} to ods.non_nhs", i, source_file_name);
    Ok(())
}