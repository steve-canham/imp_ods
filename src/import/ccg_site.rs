...
impl CCGSiteVecs{
    ...
    pub async fn store_data(&self, pool : &Pool<Postgres>) -> Result<PgQueryResult, AppError> {
        ...
        let sql = r#"INSERT INTO ods.ccg_sites (ods_code, ods_name, grouping, health_geog, 
                      city, postcode, postal_add, open_date, close_date, parent_org, join_parent_date, left_parent_date) 
...
pub async fn import_data(data_folder: &PathBuf, source_file_name: &str, pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let mut dv: CCGSiteVecs = CCGSiteVecs::new(vector_size);
            
    for result in csv_rdr.deserialize() {
    
        let source: CCGSiteLine = result?;
