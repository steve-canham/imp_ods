...
impl CCGVecs{
    ...
    pub fn add_data(&mut self, r: &CCGRec) 
    {
    ...
    pub async fn store_data(&self, pool : &Pool<Postgres>) -> Result<PgQueryResult, AppError> {
        ...
        let sql = r#"INSERT INTO ods.ccgs (ods_code, ods_name, grouping, health_geog, 
...
pub async fn import_data(data_folder: &PathBuf, source_file_name: &str, pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let mut dv: CCGVecs = CCGVecs::new(vector_size);
            
    for result in csv_rdr.deserialize() {
    
        let source: CCGLine = result?;
        ...
        dv.add_data(&ccg_rec);   // transfer data to vectors
        i+=1;    
    }
