use crate::AppError;
use crate::utils;
use crate::vectors::sha_vectors::SHAVecs;
use super::rec_structs::SHARec;
...
pub async fn import_data(data_folder: &PathBuf, source_file_name: &str, pool: &Pool<Postgres>) -> Result<(), AppError> {
    ...
    let mut dv: SHAVecs = SHAVecs::new(vector_size);
            
    for result in csv_rdr.deserialize() {
    
        let source: SHALine = result?;
        ...
        dv.add_data(&sha_rec);   // transfer data to vectors
        i +=1;    
